...
y=f(ind);   % f1,f2,f3,... (nu inteleg de ce avem nevoie fe f(indici) si nu de f(xi) unde i sunt indicii)
x=x(:);     % echivalent cu x' -> transpusa 
y=y(:);     % echivalent cu y' -> transpusa
...
%prepare diagonals
ds = dx(1:end-1);     % delta_xi-1       formula 3/pg 21/2012CursPDatEx04/ --> al treilea element din adunare
dd = dx(2:end);       % delta_xi         formula 3/pg 21/2012CursPDatEx04/ --> primul element din adunare
dp = 2*(ds+dd);                 
md = 3*(dd.*ddiv(1:end-1) + ds.*ddiv(2:end));   %rhs formula 4/pg 21/2012CursPDatEx04
...
case {1,2}
    dp1 = 2; dpn = 2;       % formula 5/pg 22/2012CursPDatEx04/ --> dp1 = 2 pt ca avem 2*m_1, dpn = 2 pt ca avem 2*m_n     
